...
inverting summer giving V_ctl = V_int_out + V_offset where V_offset is
generated by a V_TO-TRACKING REFERENCE: a third matched DMN3404L wired
diode-connected with source-degenerating R_ref_src, biased from VCC via
R_ref. This produces V_offset ≈ V_TO + 0.31 V, so V_GS_var at V_int_out=0
sits at V_TO + 0.31 V (weak triode, small R_DS, low cold-start drive)
regardless of part-to-part V_TO variation across the datasheet
V_GS(th)=1.0–2.0 V spec range. V_int_out_OP settles at ≈ -0.41 V on every
part because V_GS_var_OP (required for bridge null at R_DS≈80 Ω) is set
by V_TO + (~−0.1 V), which translates to V_int_out_OP = V_GS_var_OP -
V_offset = -0.41 V independent of V_TO.

The tracking reference replaces the earlier fixed +2.0 V V_offset which
worked for the SPICE model (V_TO_eff = +1.7 V) but would stall low-V_TO
parts (V_GS = 2.0 V into a V_TO=1.0 V part puts the channel deep in
strong inversion → R_DS ≈ 0 → no bridge drive) and overshoot high-V_TO
parts (V_GS = 2.0 V into a V_TO=2.0 V part is at threshold → R_DS large
→ high cold-start drive).

Earlier architecture history: PMOS+inverter (V_int_out=0 ⇒ V_GS=0 ⇒ R_DS=∞
⇒ |1-H|=2 ⇒ MAX cold-start drive, cause of the 121 K cold-start overshoot
on ILC1-1/7); before that JMMBFJ112 JFET (wrong-polarity bistability trap
via gate-source forward conduction).

Cold-start transient: V(T_node) starts at 300 K, V_int_out at 0, NMOS pair
in weak triode (R_DS≈6 Ω/FET at V_OD=+0.31 V), |1-H|≈0.15, bridge drive ≈
15 % of OP. As the bridge senses R_fil < target, the inverting integrator
winds V_int_out NEGATIVE, V_GS drops, R_DS grows, |1-H| grows, drive grows.
Filament heats. At balance, V_int_out_OP ≈ -0.41 V (all four tubes); V_GS_OP
at OP-required R_DS (~80 Ω/FET), all-pass corner at f0 = 1 kHz.
"""
...
def make_netlist(data_path: Path,
    ...
    # Variable-R gate DC offset. V_ctl = V_int_out + V_offset, where
    # V_offset is generated by a V_TO-tracking diode-degenerated reference
    # (Q_ref + R_ref + R_ref_src, see netlist body). Default sizes the
    # reference for V_OD_cold ≈ +0.31 V at V_int_out=0, giving R_DS ≈ 6 Ω,
    # |1-H| ≈ 0.15, P_cold ≈ 40 mW into a cold 7.7-Ω filament -- enough to
    # start heating gently while the loop engages.
    #
    # force_v_offset_v: if set, replace the diode-ref with a fixed voltage
    # source at this value. Used by V_TO-mismatch sweeps and back-compat
    # validation. Default None ⇒ use the tracking reference.
    force_v_offset_v = mc.get("force_v_offset_v", None)
    # R_ref_kohm, R_ref_src_ohm: size the V_extra above V_TO. With
    # R_ref=4.7k, R_ref_src=220, VCC=+9V → V_extra = I·R_ref_src ≈ 0.31 V.
    r_ref_kohm     = mc.get("r_ref_kohm", 4.7)
    r_ref_src_ohm  = mc.get("r_ref_src_ohm", 220.0)
...
.include {nmos_path}"""
    ...
    # V_offset reference: V_TO-tracking diode-degenerated DMN3404L (default)
    # or a fixed voltage source (when force_v_offset_v is set for diagnostics).
    if force_v_offset_v is not None:
        v_offset_block = (
            f"* Fixed V_offset (diagnostic / sweep override). Hardware: LM4040-2.0.\n"
            f"V_offset_ref v_offset_ref 0 {float(force_v_offset_v):.4g}"
        )
    else:
        # Diode-connected DMN3404L with source degeneration. Sized so
        # V_offset_ref ≈ V_TO + 0.31 V at typical V_TO=1.7 V (model).
        # KVL: VCC - I·R_ref - V_GS_diode - I·R_ref_src = 0
        #      V_offset_ref = V_GS_diode + I·R_ref_src
        # With R_ref=4.7 kΩ, R_ref_src=220 Ω, VCC=+9 V:
        #   I ≈ 1.4 mA, V_R_src ≈ 0.31 V, V_offset_ref ≈ V_TO + V_OD_diode
        #   + 0.31 V ≈ V_TO + 0.32 V (V_OD_diode ≈ 14 mV at 1.4 mA).
        # Across V_TO=1.0/1.5/2.0 V (datasheet spec): V_offset_ref shifts
        # to track V_TO, so V_GS_var at V_int_out=0 stays at V_TO + 0.31..0.36 V
        # (weak triode, low cold-start drive) regardless of part variation.
        # XM_ref must stay at the DMN3404L manufacturer model in the level-1
        # swap (validate_cap_470nf_iv6max_level1.py PRESERVE set), or the
        # tracking property is lost.
        v_offset_block = (
            f"* V_TO-tracking V_offset reference: diode-connected DMN3404L\n"
            f"* with source degeneration. V_offset_ref ≈ V_TO + 0.31 V across\n"
            f"* the DMN3404L datasheet V_GS(th)=1.0–2.0 V spread.\n"
            f"R_ref      vcc          v_offset_ref  {r_ref_kohm:.4g}k\n"
            f"R_ref_src  n_ref_src    0             {r_ref_src_ohm:.4g}\n"
            f"XM_ref     v_offset_ref v_offset_ref  n_ref_src  DMN3404L"
        )
...
* === Variable-R gate drive: non-inverting summer with V_TO-tracking shift ===
* V_ctl = V_int_out + V_offset, with V_offset generated by a V_TO-tracking
* reference (XM_ref + R_ref + R_ref_src below). At cold start V_int_out=0,
* so V_GS_var = V_offset = V_TO + ~0.31 V across the DMN3404L
* V_GS(th)=1.0-2.0 V datasheet spec, regardless of which sample is fitted.
* This puts the variable-R FETs in weak triode (R_DS ≈ 6 Ω/FET, |1-H| ≈
* 0.15, P_cold ≈ 40 mW) so the filament heats gently while the loop
* engages. As V_int_out winds negative, V_GS drops, R_DS grows, drive
* grows. V_int_out_OP settles at ≈ -0.41 V across all V_TO values
* because V_GS_var_OP (required for bridge null) is set by R_DS_OP, which
* depends only on V_OD_var = V_int_out + (V_offset - V_TO) ≈ V_int_out +
* 0.31 V -- V_TO cancels exactly when Q_var and Q_ref are matched.
*
* In hardware, Q_ref is a 3rd matched DMN3404L (same reel as Q_var1/Q_var2)
* wired diode-connected (drain tied to gate) with R_ref_src source
* degeneration. R_ref biases ~1.4 mA from VCC; R_ref_src drops 0.31 V at
* that current, lifting V_offset_ref to V_GS_diode + 0.31 V ≈ V_TO + 0.32 V.
* Tempco of V_TO (~-5 mV/K) tracks automatically across Q_ref and Q_var.
{v_offset_block}
