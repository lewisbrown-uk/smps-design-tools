...
| **U6 (J_var, J_ref)** | All-pass variable resistor + self-biased V_p tracking reference (LS844: monolithic dual N-JFET) | LS844 | Linear Integrated Systems | SOT-23-6 (dual on die) | LS844-SOT-23-6 | (LIS direct or DigiKey) | 1 | 2.50 | One die (J_var) is the variable-R in the all-pass: drain at `v_drv_atten`, source at `n_ap_plus`, gate at `V_ctl`. Other die (J_ref) is self-biased: drain to VCC via R_d_ref, source to GND via R_src_ref, gate to GND. Generates V_source_ref ≈ +0.1 V (positive); XU_sum subtracts to produce V_ctl = V_int_out − V_source_ref. Matched on-die: ΔV_GS ≤ 5 mV, I_DSS within 5 %, drift 10 µV/°C. The matching means V_GS_var-V_p tracks across the LS844 V_GS(off) spec range (1.0–3.5 V). **No body diode** (the reason for switching from back-to-back DMN3404L MOSFETs): clean symmetric all-pass operation, H2 < 0.4 % vs ~16–175 % with the MOSFET arch. |
| **R_d_ref**        | Drain pull-up resistor for J_ref                  | 10 kΩ            | 1 %             | 0805 / through-hole | (generic) | (generic)           | 1   | 0.01     | Sets the Q_ref drain bias point; current-limits the self-bias loop to ~0.9 mA so V_source_ref settles at 0.08–0.1 V even on the highest-I_DSS LS844 parts. |
| **R_src_ref**      | Source-degeneration resistor for J_ref            | 100 Ω            | 1 %             | 0805 / through-hole | (generic) | (generic)           | 1   | 0.01     | Sets V_source_ref = I·R_src_ref ≈ 0.086 V at typical LS844 (V_p=-2 V, I_DSS=5 mA). Small value chosen so V_GS_var at cold-start (V_int_out=0) is near 0 → low R_DS → modest cold-start bridge drive (the JFET cannot achieve as-small cold-start drive as the MOSFET arch could; this is the natural cost of the JFET architecture). |
| **C_btd, C_bts**   | AC-coupling caps for bootstrap                    | 100 nF           | 10 %, X7R       | 0805 / through-hole | (generic) | (generic)           | 2   | 0.04     | AC-couple `v_drv_atten` and `n_ap_plus` into the XU_sum +IN node. HP cut-off 1/(2π·100k·100n) ≈ 16 Hz — well below f₀=1 kHz so bootstrap is full strength at the operating frequency, and well above the loop bandwidth so DC operation is unaffected. |
| **R_btd, R_bts**   | Bootstrap-injection series resistors              | 100 kΩ each      | 1 %             | 0805 / through-hole | (generic) | (generic)           | 2   | 0.02     | Combined with R_sum_a/R_sum_gnd (100 kΩ each from V_int_out and GND), produces V_ctl_AC = (V_drv+V_ap)/2 = V_avg, which makes V_GS_AC = V_DS/2 — exactly the V_DS/2-tracking bootstrap that cancels the V_DS-dependent term in R_DS(V_GS, V_DS). Reduces H2 from ~7–11 % (no bootstrap) to ~0.3 % (with bootstrap) at the same loop operating point. |
| **U7 (XU_log_dem)** | Log conformer op-amp + BJT (signed-log demod between demod & integrator) | 1× op-amp channel + 1× BJT (MMBT3904 / MMDT3904 matched pair for tempco) | TI / Nexperia | SOIC-8 / SOT-23 | (use TLV9152 SOT-23-5 or 1 channel of a TLV9154 quad) | — | 1 set | 0.30 | Implements V_out = K · sign(V_in) · ln(1 + |V_in|/v_eps). High small-signal gain (K/v_eps = log_gain_K, see TUBES dict) gives fast OP settling on the 1 V tubes (10-14× speedup vs linear demod); 1/(1+|V|/v_eps) compression at large signals bounds cold-start integrator wind rate, keeping T_pk overshoot to <4 K across all tubes. Per-tube log_gain_K: {IV-18:10, IV-6:7.5, ILC1-1/7:5, ILC1-1/8:7.5} — implemented as one resistor that differs per tube in the conformer's V_in/I converter stage. Hardware: classic transdiode log conformer (op-amp with BJT collector-base shorted in feedback). MMDT3904 matched-pair BJT preferred for tempco self-cancellation. |
...
| 1× LS844 (variable-R + V_p tracking ref) | 2.50       |
| 1× 10 k (R_d_ref) + 1× 100 (R_src_ref)   | 0.02       |
| 2× 100 nF (C_btd, C_bts bootstrap caps)  | 0.04       |
| 2× 100 k (R_btd, R_bts bootstrap Rs)     | 0.02       |
| Log conformer (BJT + op-amp ch + 1 R)    | 0.30       |
