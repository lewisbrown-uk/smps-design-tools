...
| **M_var1, M_var2** | All-pass variable resistor (back-to-back NMOS, `XM_var1`/`XM_var2`) | DMN3404L-7   | Diodes Inc. | SOT-23  | DMN3404L-7DICT-ND | 621-DMN3404L-7    | 2   | 0.35 ea  | Two NMOS in series, sources tied at `n_var_mid`, gates tied to `V_ctl = V_int_out + V_offset_ref` (see M_ref entry below). With V_offset_ref ≈ V_TO + 0.31 V (tracking reference), V_int_out = 0 puts V_GS into weak triode (R_DS ~6 Ω/FET, |1-H| ~0.15), giving small cold-start drive while still developing a usable bridge signal for the loop to engage. V_int_out_OP ≈ -0.41 V across all four tubes and across the V_GS(th)=1.0–2.0 V datasheet spread. Same part as the bridge driver — order matched / same-reel with M_ref. |
...
| **M_ref**          | V_TO-tracking reference FET (diode-connected, source-degenerated) | DMN3404L-7    | Diodes Inc. | SOT-23  | DMN3404L-7DICT-ND | 621-DMN3404L-7    | 1   | 0.35     | 3rd matched DMN3404L wired drain-to-gate (diode-connected) with R_ref biasing from VCC and R_ref_src in the source path. Generates V_offset_ref = V_GS_diode + I·R_ref_src ≈ V_TO + 0.31 V at ~1.4 mA bias. Tracks Q_var1/Q_var2's V_TO automatically across the datasheet V_GS(th)=1.0–2.0 V spread and cancels V_TO tempco (~−5 mV/K). **Must be from the same reel as M_var1/M_var2** — matching better than ±50 mV V_TO keeps V_int_out_OP within the integrator's clamp range. SOT-23 layout: place M_ref next to M_var1/M_var2 for thermal coupling. |
| **R_ref**          | V_offset-ref bias resistor (VCC → diode-connected M_ref) | 4.7 kΩ           | 1 %             | 0805 / through-hole | (generic) | (generic)           | 1   | 0.01     | Sets bias current I ≈ (VCC − V_offset_ref)/(R_ref + R_ref_src) ≈ 1.4 mA. Tolerance not critical: V_offset_ref's V_extra ≈ I·R_ref_src varies <±5 mV across 1 % R tolerance, well below the closed-loop demod resolution. |
| **R_ref_src**      | V_offset-ref source-degeneration resistor (M_ref source → GND) | 220 Ω            | 1 %             | 0805 / through-hole | (generic) | (generic)           | 1   | 0.01     | Sets V_extra = I·R_ref_src ≈ 0.31 V on top of V_GS_diode. Picks the V_OD of the variable-R FETs at cold-start (V_int_out=0). |
...
| 3× DMN3404L (M_var1, M_var2, M_ref)      | 1.05       |
| 1× 1 MΩ (R_var_mid_bias)                 | 0.01       |
| 1× 4.7 k (R_ref) + 1× 220 (R_ref_src)    | 0.02       |
