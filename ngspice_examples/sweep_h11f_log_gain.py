...
# Load prior results from CSV (Schottky clipper run, gains 1..10)
import csv as _csv
CACHED = []
csv_path = HERE / "sweep_h11f_log_gain_results.csv"
if csv_path.exists():
    with open(csv_path) as fh:
        rd = _csv.reader(fh)
        next(rd, None)  # header
        for row in rd:
            CACHED.append((row[0], float(row[1]), float(row[2]), float(row[3]),
                            float(row[4]), float(row[5]), float(row[6]),
                            float(row[7]), float(row[8])))

NEW_CASES = []
# Extended H11F arch sweep:
#   - IV-18 wants gain≤1.5 (overshoot rises sharply past gain=1)
#   - others (IV-6, ILC1-1/7, ILC1-1/8) want gain HIGH (curves still
#     descending at gain=10 in prior sweep)
GAIN_GRID = {
    "iv18":    (1.0, 1.2, 1.5, 1.7, 2.0, 3.0, 5.0, 7.5, 10.0),
    "iv6":     (1.0, 2.0, 3.0, 5.0, 7.5, 10.0, 15.0, 20.0, 30.0),
    "ilc11_7": (1.0, 2.0, 3.0, 5.0, 7.5, 10.0, 15.0, 20.0, 30.0),
    "ilc11_8": (1.0, 2.0, 3.0, 5.0, 7.5, 10.0, 15.0, 20.0, 30.0),
}
for tube, gains in GAIN_GRID.items():
    for g in gains:
...
def run(case):
    ...
    mc["v_eps_log"]    = 5e-3
    mc["nonlin_type"]  = "log"
    mc["log_clip_type"]= "schottky"  # BAT54 anti-parallel real clipper
    ...
    cir = tcl.WORK / f"swH_{label}.cir"
    dat = tcl.WORK / f"swH_{label}.data"
...
def main():
    ...
    axes[0].set_title("H11F log demod sweep: gain=1 → 10 (eps=5 mV, signed-log) on H11F arch")
    ...
    fig.tight_layout()
    png_path = HERE / "sweep_h11f_log_gain.png"
